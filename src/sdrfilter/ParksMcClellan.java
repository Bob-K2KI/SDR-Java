...
public class ParksMcClellan
{
    ...
    public static int estimatedOrder(FilterSpecification s) 
    {
        float fs = 2*s.fN;
        return estimatedOrder(s.ft/fs, s.dBatten, s.dBripple);
        
    }

    public static int estimatedOrder(float trband, float atten, float ripple) 
    {
        
//        float deltaP = 0.5f*(1.0f - (float)Math.pow(10.0f, -0.05f*ripple));
        float deltaP = (1.0f - (float)Math.pow(10.0f, -0.05f*ripple));
        float deltaS = (float)Math.pow(10.0f, -0.05f*atten);
        // estimate required filter order using Kaiser formula
        return (int) Math.round((-10.0f*(float)Math.log10(deltaP*deltaS) - 13)/(14.6f*trband));
    }

    public static float[] design(FilterSpecification s) {
        if ( s.order < 1 ) {
        ...
        float fs = 2*s.fN;
        return design(s.shape, s.f1/fs, s.f2/fs, s.ft/fs, s.dBatten, s.dBripple, s.order);
    }

    public static float[] design(String shape, float f1, float f2, float trband, float atten, float ripple, int order) {
        
        
        int numBands = 2, numTaps;
        ...
        if (shape == "BPF" || shape == "NOTCH") numBands = 3;
        ...
        private static void calcParms(int r, int ext[], double grid[], double d[], double w[],
         ...
         *********************/
        private static double[] freqSample(double A[], int symm, int numtaps) {
          double x, val;
          int N = numtaps;
          double M = (N-1.0)/2.0;
        ...
        private static double[] remez(int numtaps,
          ...
          {
              MainScreen.setArea("Filter error Reached maximum iteration count.\nResults may be bad.\n");
              DialogBean badFilter = new DialogBean("Reached maximum iteration count. Results may be bad.");
          }
          ...
          return freqSample(taps, symmetry, numtaps);
